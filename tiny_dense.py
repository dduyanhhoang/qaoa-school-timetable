...
# LOAD GRAPH METADATA
# --------------------------
with open('dataset/tiny_graph_dense.pickle', 'rb') as f:
    G_METADATA = pickle.load(f)

# --------------------------
# TEACHER MAPPING
# --------------------------
REAL_TEACHERS = list(DF_SKILLS.index)  # ['T_A', 'T_B']
...
if required_colors > NUM_REAL:
    ...
    print(f"\n[!] VALIDATION SUCCESS: Graph requires {diff} more teachers.")
    print("Creating dummy teachers:")

    for i in range(diff):
        dummy_id = NUM_REAL + i
        TEACHER_MAP[dummy_id] = f"DUMMY_{i + 1}"
...
def main():
    print("\nStarting program for dense version of Tiny dataset\n")
    school = "Tiny_Dense"
    G = G_METADATA
